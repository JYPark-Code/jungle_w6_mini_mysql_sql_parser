...
static void print_welcome_banner(void) {
    ...
    size_t kitty_count = sizeof(kitty) / sizeof(kitty[0]);
    size_t welcome_count = sizeof(welcome) / sizeof(welcome[0]);
    size_t parser_count = sizeof(sql_parser) / sizeof(sql_parser[0]);

    putchar('\n');
    for (size_t i = 0; i < kitty_count; ++i) {
        const char *right = "";
        if (i >= 10 && i < 10 + welcome_count) {
            right = welcome[i - 10];
        } else if (i >= 22 && i < 22 + parser_count) {
            right = sql_parser[i - 22];
        }
        printf("%-84s   %s\n", kitty[i], right);
    }
    printf("Hello Kitty welcomes you to MiniSQL!\n\n");
}
